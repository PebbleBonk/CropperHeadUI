...
var ImageUploader = function(config) {
    ...
    if (!config || (!config.firingElement) || (!config.firingElement.getAttribute)) {
        throw new Error('Config object passed to ImageUploader constructor must include "firingElement" ');
    }
    this.setConfig(config);

    var This = this;

    this.config.firingElement.addEventListener('click', function() {
        var fileArray = [];
        var cursor = 0;
...
ImageUploader.prototype.handleFileSelection = function(file, completionCallback) {
    ...
    // var img = this.config.hiddenImgElement;
